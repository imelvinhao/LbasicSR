...
class VideoBaseModel(SRModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        if with_metrics:
            if (
                not hasattr(self, "metric_results") or self.metric_results
            ):  # only execute in the first run X
                self.metric_results = {}
        ...
        for idx in range(rank, len(dataset), world_size):
            val_data = dataset[
                idx
            ]  # dict{'lq': Tensor TCHW, 'gt': Tensor CHW, 'folder': 'calendar', 'idx': '0/41', border: 1, 'lq_path': str}
